...
def astar_search(initial_state, goal):
    open_list = []
    heapq.heappush(open_list, (0 + manhattan_distance(initial_state, goal), initial_state, []))
    seen_states = set()

    while open_list:
        current_f, current_state, path = heapq.heappop(open_list)
...
def main():
    ...
    solution_path = astar_search(initial_state, goal)
